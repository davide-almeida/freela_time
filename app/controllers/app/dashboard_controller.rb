class App::DashboardController < AppController
  def index
    ...
    @in_payment_json = @in_payments_reports.joins(:in_parcels).where('status = ?', 1).where("due_date BETWEEN ? AND ?", Time.zone.now.at_beginning_of_year, Time.zone.now.at_end_of_year)
    value_array = []
